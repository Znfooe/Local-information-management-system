...
// DB Setup
type Data = {
  apis: Array<{ id: number; name: string; url: string; key: string; description: string }>;
  credentials: Array<{ id: number; site_name: string; url: string; username: string; password?: string; created_at: string }>;
  settings: { apiKey?: string; model?: string; language?: string; theme?: string; provider?: string };
  sessions: Array<{ id: string; title: string; messages: any[]; updatedAt: number }>;
}
...
ipcMain.handle('get-apis', async (event, search) => {
  await db.read();
  if (search) {
    return db.data.apis.filter((a: any) => a.name.toLowerCase().includes(search.toLowerCase()));
  }
  return db.data.apis;
});
...
ipcMain.handle('save-api', async (event, api) => {
  ...
  await db.read();
  if (api.id) {
    const index = db.data.apis.findIndex((a: any) => a.id === api.id);
    if (index !== -1) {
...
ipcMain.handle('delete-api', async (event, id) => {
  await db.read();
  db.data.apis = db.data.apis.filter((a: any) => a.id !== id);
  await db.write();
  return true;
});
...
ipcMain.handle('get-creds', async (event, search) => {
  await db.read();
  if (search) {
    return db.data.credentials.filter((c: any) => c.site_name.toLowerCase().includes(search.toLowerCase()));
  }
...
ipcMain.handle('save-cred', async (event, cred) => {
  ...
  await db.read();
  if (cred.id) {
    const index = db.data.credentials.findIndex((c: any) => c.id === cred.id);
    if (index !== -1) {
...
ipcMain.handle('delete-cred', async (event, id) => {
  await db.read();
  db.data.credentials = db.data.credentials.filter((c: any) => c.id !== id);
  await db.write();
  return true;
});
...
ipcMain.handle('save-session', async (event, session) => {
  ...
  const index = db.data.sessions.findIndex((s: any) => s.id === session.id);
  if (index !== -1) {
...
ipcMain.handle('delete-session', async (event, id) => {
  await db.read();
  if (!db.data.sessions) return false;
  db.data.sessions = db.data.sessions.filter((s: any) => s.id !== id);
  await db.write();
  return true;
});
